...
use super::{PocketPrecision, VoiceStyle};
...
impl AprilPocketTts {
    pub(crate) fn load(
        ...
        let (flow_main, flow, mimi_decoder) = match precision {
            PocketPrecision::Fp32 => (FILE_FLOW_MAIN_FP32, FILE_FLOW_FP32, FILE_MIMI_DECODER_FP32),
            PocketPrecision::Int8 => (FILE_FLOW_MAIN_INT8, FILE_FLOW_INT8, FILE_MIMI_DECODER_INT8),
        };

        Ok(Self {
            // Match the pinned upstream precision selector: voice encoding and
            // text conditioning remain full precision in both modes.
...
mod tests {
    ...
    fn loader_splits_oversized_prompts_at_bundle_token_limit() {
        let dir = std::env::var("BUZZ_POCKET_TEST_MODEL_DIR")
            .expect("set BUZZ_POCKET_TEST_MODEL_DIR to the verified April bundle");
        let engine = AprilPocketTts::load(Path::new(&dir), PocketPrecision::Fp32, 1)
            .expect("load April bundle");
